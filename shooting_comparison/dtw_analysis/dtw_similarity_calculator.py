...
# Handle new Rising DTW features
        ...
        for key in feature1.keys():
            ...
            # Detailed debugging for each subfeature of ball_wrist_trajectory
            if feature_name == 'ball_wrist_trajectory':
        ...
        # Debugging: Output ball_wrist_trajectory subfeature similarities
        if feature_name == 'ball_wrist_trajectory':
            ...
            # Output detailed information for each subfeature
    ...
    def _calculate_2d_trajectory_similarity(self, traj1: List, traj2: List, 
        ...
        # ball_wrist_trajectory related debugging
    ...
    def _calculate_1d_series_similarity(self, series1: List, series2: List,
        ...
        # ball_wrist_distance related debugging
    ...
    def _distance_to_similarity(self, distance: float, feature_type: str) -> float:
        ...
        # Extreme differentiation: Adjust similarity changes based on distance very dramatically
        if normalized_distance <= 0.1:
            # Very similar case: Very high similarity (99-100%)
            similarity = 100 * (1 - normalized_distance * 0.1)
        elif normalized_distance <= 0.25:
            # Similar case: High similarity (85-99%)
            similarity = 100 * (0.99 - (normalized_distance - 0.1) * 0.93)
        elif normalized_distance <= 0.5:
            # Moderately similar case: Medium similarity (60-85%)
            similarity = 100 * (0.85 - (normalized_distance - 0.25) * 1.0)
        elif normalized_distance <= 0.8:
            # Different case: Low similarity (30-60%)
            similarity = 100 * (0.6 - (normalized_distance - 0.5) * 1.0)
        else:
            # Very different case: Very low similarity (10-30%)
            similarity = 100 * max(0.1, 0.3 - (normalized_distance - 0.8) * 0.4)
        
        # Ensure minimum similarity score
        final_similarity = max(10.0, min(100.0, similarity))
        
        # Output debugging information (always output for ball_wrist_special type)
    ...
    def compute_global_score(self, user_data: Dict, ref_data: Dict, config: Dict = None) -> Dict:
        """
        Calculate global motion similarity for the entire sequence.
        An improved method replacing the feature-based similarity.
        
        Args:
            user_data: User video data (including wrist_y, com_y, fps, phases)
            ref_data: Reference video data (including wrist_y, com_y, fps, phases)
            config: Configuration parameters (weights, etc.)
            
        Returns:
            Global similarity score and detailed analysis results
        """
        if config is None:
            config = {
                'w_dtw_wrist': 0.25,
        ...
        try:
            ...
            # 2. Calculate Phase Timing Vector
            ...
            # 6. Calculate final global score
            global_score = (
    ...
    def _smooth_motion_data(self, data: Dict) -> Dict:
        """Smooth motion data (Savitzky-Golay or Butterworth filter)"""
        smoothed = {}
    ...
    def _calculate_phase_timing_similarity(self, user_data: Dict, ref_data: Dict) -> float:
        ...
        try:
            ...
            similarity = max(0, 100 - (l1_distance * 200))  # Convert L1 distance to similarity
    ...
    def _calculate_global_dtw(self, series1: List[float], series2: List[float]) -> float:
        """Calculate global DTW similarity"""
    ...
    def _calculate_boundary_penalty(self, user_data: Dict, ref_data: Dict) -> float:
        """Calculate boundary smoothness penalty"""
        try:
            penalty = 0.0
            ...
            for phase1, phase2 in phase_transitions:
                if phase1 in user_phases and phase2 in user_phases:
                    # Check for changes at the boundary (example)
        ...
        try:
            ...
            if phase1_end < len(wrist_y) and phase2_start < len(wrist_y):
                ...
                if phase1_end > 0 and phase2_start < len(wrist_y) - 1:
                    ...
                    return max(0, 1 - slope_diff)  # The greater the slope difference, the less smooth
