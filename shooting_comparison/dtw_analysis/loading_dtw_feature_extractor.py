...
class LoadingDTWFeatureExtractor(SafeCoordinateMixin):
    ...
    Focuses on:
    A. loading_leg_kinematics (40%): 다리 각도 변화 패턴, 변화율, 가속도, 비대칭성
    B. loading_upper_body_dynamics (35%): 어깨 기울기, 상체 기울기, 거리 변화, 회전
    C. loading_timing_patterns (25%): 타이밍 패턴 및 전환점 분석
    """
    ...
    def _extract_upper_body_dynamics(self, loading_frames: List[Dict]) -> Dict:
        ...
        for frame in loading_frames:
            ...
            # Calculate torso rotation (상체 회전각도)
    ...
    def _extract_timing_patterns(self, loading_frames: List[Dict]) -> Dict:
        ...
        Features:
        - transition_start: 로딩 단계에서 다리 max각도까지 도달한 시점
        - transition_end: max각도에서 로딩 끝까지 걸린시간  
        - transition_duration: max각도에서 일정 유지된 시간
        - 단계별 타이밍 비율
        """
    ...
    def _calculate_torso_rotation(self, pose: Dict) -> Optional[float]:
        """Calculate torso rotation (어깨선과 엉덩이선 사이의 회전 각도)"""
        try:
            left_shoulder = pose.get('left_shoulder', {})
            right_shoulder = pose.get('right_shoulder', {})
