...
class RisingDTWFeatureExtractor(SafeCoordinateMixin):
    ...
    Focuses on:
    A. rising_windup_kinematics (40%): 궤적 패턴, 속도, 가속도, 곡률 변화
    B. rising_jump_dynamics (35%): 점프 역학, 상체/다리 변화 패턴
    C. rising_timing_patterns (25%): 타이밍 패턴 및 단계별 비율
    """
    ...
    def __init__(self):
        ...
        self.interpolation_frames = 20  # windup 궤적을 20 프레임으로 정규화
    ...
    def _extract_timing_patterns(self, rising_frames: List[Dict]) -> Dict:
        ...
        Features:
        - dip_point → setup_point 타이밍
        - setup_point → release 타이밍  
        - 전체 라이징 타이밍
        - 윈드업 비율 변화
        - 단계별 타이밍 비율
        """
