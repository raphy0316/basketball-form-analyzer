...
class BasketballShootingAnalyzer:
    def __init__(self, fps=30, interactive=True):
    ...
    def load_associated_data_api_helper(self, base_name) -> bool:
        """
        Load associated data for the received frames.
        """

        # pose_original_json = os.path.join(self.extracted_data_dir, f"demo_pose_original.json")
        # ball_original_json = os.path.join(self.extracted_data_dir, f"demo_ball_original.json")

        pose_files = glob.glob(os.path.join(self.extracted_data_dir, f"{base_name}_pose_original*.json"))
        file_path = os.path.join(self.extracted_data_dir, f"{base_name}_pose_original*.json")
        print(f"Loading original pose data from: {file_path}")
        print("pose_File: ", pose_files)
        if pose_files:
            ...
            try:
                with open(ball_file, 'r', encoding='utf-8') as f:
                    ...
                    if isinstance(data, dict) and "ball_trajectory" in data:
                        ...
                        self.video_fps = data.get("metadata").get("fps", self.video_fps)
